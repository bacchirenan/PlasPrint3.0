...
function calc(opts) {
    ...
    let tOut = totalN > 0 ? (totalOee / totalN * 100) : 0;
    ...
    let dist = Math.sqrt(
        Math.pow(tOut - 41.38, 2) +
        Math.pow(n28 - 81.24, 2) +
        Math.pow(n180 - 48.71, 2) +
        Math.pow(n181 - 52.91, 2) +
        Math.pow(n182 - 58.03, 2)
    );

    return { dist, tOut, n28, n180, n181, n182, opts };
}
...
for (let rT3 of bools)
    for (let rTV of bools)
        for (let rHV of bools)
            for (let rH of bools)
                for (let rP of bools) {
                    res.push(calc({ rT3, rTV, rHV, rH, rP }));
                }
res.sort((a, b) => a.dist - b.dist);
console.log('BEST MATCHES:');
for (let i = 0; i < 3; i++) {
    console.log(`DIST: ${res[i].dist.toFixed(2)} | T: ${res[i].tOut.toFixed(2)} | 28: ${res[i].n28.toFixed(2)} | 180: ${res[i].n180.toFixed(2)} | 181: ${res[i].n181.toFixed(2)} | 182: ${res[i].n182.toFixed(2)}`);
